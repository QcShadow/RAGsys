...
注意：
- 这是“规则优先 + 失败兜底”的方法，不会对所有文件都调用LLM，成本更可控
- LLM prompt 严格排除主题列表/空泛困难描述，尽量避免“强行找疑问”
"""
...
class Candidate:
    ...
    llm: Optional[Dict[str, Any]] = None  # 规则抽取为 None；LLM兜底可填
...
def main():
    ...
    parser.add_argument("--llm_timeout", type=int, default=120, help="LLM request timeout seconds")
    ...
    for p in paths:
        try:
            ...
            # 2) 若规则为0，调用LLM兜底（LLM允许输出[]，所以不会强行找）
            ...
            results.append({
                ...
                "fallback_used": used_fallback,  # 额外字段：便于你统计有多少文件触发了兜底
            })
