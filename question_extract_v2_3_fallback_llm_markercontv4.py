...
# -*- coding: utf-8 -*-
"""
question_extract_v2_3_fallback_llm_fixed_full.py

主流程：
1) 先用 v2.3 规则抽取疑问候选（疑问段硬门控）
...
本版关键修复（解决“符丽”那种：问题：1.xxx 另起一行 2.xxx 漏抽的问题）：
- 当检测到“显式 marker 行”（如 “问题：1.xxx”）时：
  A) 先抽取该行尾部的内容（marker_parts）
  B) 再向后吸收“紧随其后的连续编号行”（2.xxx / 3.xxx ...）作为额外候选
    -> 这样就不会漏掉 “2.各头文件及库函数的意义和用法”

其他修复：
A) DOCX 读取保留段落内换行（splitlines）
B) 支持 “问题/疑问/困惑/疑惑” 单独一行标题 -> 连续吸收后续枚举条目（允许名词化）
C) 避免“心得：...区别...”误抽取为问题（心得行若无问号/困惑词/marker则跳过）

运行示例：
python question_extract_v2_3_fallback_llm_fixed_full.py norm_dataset --out out.json
调试某文件：
python question_extract_v2_3_fallback_llm_fixed_full.py norm_dataset --debug_file 符丽 --max_preview 20
"""
...
import requests
from llm_retry import post_with_retry_forever
# ---------------------
# LLM 配置：建议用环境变量覆盖
# ---------------------
# 你可以直接在系统环境变量里设置 OPENAI_API_KEY / OPENAI_BASE_URL / OPENAI_MODEL
...
def looks_like_section_title(line: str) -> bool:
    """
    注意：这里尽量不要把“2.xxx”当标题（否则会影响续行枚举）
    我们只把这些算标题：
      - 明确的段落提示词
      - 第X章/节
...
"""
    ...
    r = post_with_retry_forever(
        url,
        headers=headers,
        payload=payload,
        timeout=timeout,
        session=sess,
        print_prefix=f"[LLM-RETRY][extract][{file_name}]",
    )
    data = r.json()
...
def main():
    ...
    parser.add_argument("--out", default="question_candidates_v2_3_fallback_fixed_full.json", help="output json filename")
